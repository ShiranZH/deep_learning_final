...
from util.mobile import Mobile, hswish, MobileDown
from util.former import Former
from util.bridge import Mobile2Former, Former2Mobile
from util.config import config_294, config_508, config_52
...
class MobileFormer(nn.Module):
    def __init__(self, cfg):
        ...
        # self.linear_in = exp + cfg['embed']
        self.head = nn.Sequential(
            nn.Linear(exp + cfg['embed'], cfg['fc1']),
            # nn.Linear(self.linear_in, cfg['fc1']),
            hswish(),
    ...
    def forward(self, x):
        b, _, _, _ = x.shape
        # print(self.token.shape)  # torch.Size([1, 6, 192])
        z = self.token.repeat(b, 1, 1)
        # print(z.shape)  # torch.Size([256, 6, 192])
        ...
        # x = self.bn(self.conv(x)).view(b, -1)
        z = z[:, 0, :].view(b, -1)
        out = torch.cat((x, z), -1)
        # self.linear_in = out.shape
        # print('out', out.shape)
